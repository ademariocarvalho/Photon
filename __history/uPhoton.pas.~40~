...
begin
  ...
  begin
    ...
    try
      //k -> calibração - ajuste para dimensão real da pcb
      k:=StrToFloat(edtConfig.Text); // 0.239271255; <OBS: no conversão usa virgula em vez de ponto>
...
begin
  ...
  begin
    // ExtractFilePath(ParamStr(0))-->> diretório do executável do projeto
    ArquivoINI := TIniFile.Create(ExtractFilePath(ParamStr(0))+'photon.ini');
